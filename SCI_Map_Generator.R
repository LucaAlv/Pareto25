...
dir.sci_dat_gadm1_nuts3_counties <- "Data/gadm1_nuts3_counties-gadm1_nuts3_counties - FB Social Connectedness Index - October 2021.tsv"
...
shapes_in <- readRDS("Data/gadm_based_shapefiles/rds_format/gadm1_nuts3_counties.Rds")
...
# India with all regions, scaling from 5th percentile
shapes_simple_IND <- shapes_in %>%
  filter(country == "IND") %>% 
  st_simplify(preserveTopology=TRUE, dTolerance = .0025)

# plot_sci(sci_dat=sci_dat, shapefiles=shapes_simple_IND,
#         country="IND",
#         output_folder="output/within_IND", scale_from_ptile=5)